...
myApp.controller("BackgroundCtrl", function($interval, $rootScope, $log, chrome, pingProcessor){
    ...
    $rootScope.$on("pingsInProgress", function(e, viewModels) {
        ...
        if (notFinishedCount > 0) {
            if (successCount > 0) {
                if (failedCount > 0) {
                    message = successCount + " Devices connected, " + failedCount + " Devices disconnected, " + notFinishedCount + " pending";
                } else {
                    message = successCount + " Devices connected, " + notFinishedCount + " pending";
                }
            } else {
                message = "No Devices disconnected, " + failedCount + " Devices disconnected, " + notFinishedCount + " pending";
            }
    ...
    $rootScope.$on("allPingsDone", function(e, viewModels) {
        ...
        if (!failed.length) {
            ...
            chrome.notifications.create({
                ...
                title: "Ping complete",
                message: "All " + viewModels.length + " Devices connected"
            });
        } else {
            chrome.notifications.create({
                type: "list",
                iconUrl: "img/icon_fail_160.png",
                title: failed.length + " of " + viewModels.length + " Devices disconnected",
                message: "",
